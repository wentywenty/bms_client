...
class BmsOta:
    ...
    def run(self):
        if not os.path.exists(self.bin_path):
            self.log(f"Error: Firmware file {self.bin_path} not found", "\033[1;31m")
            return False

        with open(self.bin_path, "rb") as f:
            self.bin_data = f.read()
        
        self.log(f"Starting firmware update: {self.bin_path} ({len(self.bin_data)} bytes)")

        self.log("Stopping bms.service to acquire serial port...")
        ...
        try:
            ...
            self.log("Step 1: Sending update request...")
            req_data = b'\x30' * 6
            self.send_frame(0x11, req_data)
            resp = self.read_response()
            if not resp or resp[0] != 0x00:
                self.log(f"Request denied: Status code 0x{resp[0] if resp else 'NULL':02X}", "\033[1;31m")
                return False

            self.log("Step 2: Sending firmware metadata...")
            ...
            self.log("\nUpdate complete! Waiting for BMS reset...", "\033[1;32m")
            time.sleep(5)
            return True

        finally:
            if self.ser: self.ser.close()
            self.log("Restarting bms.service...")
